...
def main(args):
    ...
    for s_id, seed in enumerate(range(args.seed, args.seed + args.num_seed)):
        ...
        train_case = case_types[:int(len(case_types) * 0.8)]
        val_case = case_types[int(len(case_types) * 0.8):int(len(case_types) * 0.9)]
        test_case = case_types[int(len(case_types) * 0.9):]

        train_g_list, val_g_list, test_g_list = [], [], []
        for data in G_list:
            case_number = data['case_number']
            if case_number in train_case:
                train_g_list.append(data)
            elif case_number in val_case:
                val_g_list.append(data)
            elif case_number in test_case:
                test_g_list.append(data)
        
        class DataModule(L.LightningDataModule):
            def train_dataloader(self):
                return PyGDataLoader(train_g_list, batch_size=args.batch_size, num_workers=24, shuffle=True)
            def val_dataloader(self):
                return PyGDataLoader(val_g_list, batch_size=args.batch_size, num_workers=24, shuffle=False)
            def test_dataloader(self):
                return PyGDataLoader(test_g_list, batch_size=args.batch_size, num_workers=24, shuffle=False)
...
if __name__ == '__main__':
    ...
    def parse_args():
        ...
        parser.add_argument("--model", type=str, default='gnopb')
        ...
        parser.add_argument("--summary", type=str, default=f'[table1]_unseen_params')
