...
from torch.nn import Linear, Sequential, ReLU, SiLU
...
class Model(pl.LightningModule):
    def __init__(self, args, sample_data):
        ...
        self.cond_encoder = Sequential(Linear(sample_data.cond_feat.shape[1], 2 * args.hidden_dim),
                            SiLU(),
                            Linear(2 * args.hidden_dim, 2 * args.hidden_dim),
                            )
        self.node_encoder = Sequential(Linear(sample_data.x.shape[1], 2 * args.hidden_dim),
                            SiLU(),
                            Linear(2 * args.hidden_dim, 2 * args.hidden_dim),
                            )
        
        self.encoder = TransformerConv(beta=False, in_channels=2 * args.hidden_dim, out_channels=args.hidden_dim, edge_dim=sample_data.edge_attr.shape[1], heads=1)

        self.decoder = Sequential(Linear(args.hidden_dim, 2 * args.hidden_dim),
                            SiLU(),
                            Linear(2 * args.hidden_dim, 1),
                            )
    ...
    def forward(self, data):
        ...
        h_cond = self.cond_encoder(data.cond_feat)
        h = h_node * h_cond[data.batch]

        pred, attention = self.encoder(x=h, edge_index=data.edge_index, edge_attr=data.edge_attr, return_attention_weights=True)
        pred = self.decoder(pred)

        if self.args.return_weight == 1:
