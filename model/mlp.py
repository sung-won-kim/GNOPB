...
class Model(pl.LightningModule):
    def __init__(self, args, sample_data):
        ...
        self.encoder = GATConv(in_channels=(sample_data.x.shape[1]+sample_data.cond_feat.shape[1]), out_channels=args.hidden_dim, heads=1)
    ...
    def forward(self, data):
        ...
        node_feat = torch.cat((data.x, cond_feat_expanded), dim=1)

        data.x = node_feat

        # Generate isolated edge index for each graph
        data.edge_index = torch.empty((2, 0), dtype=torch.long).to(data.x.device)  
